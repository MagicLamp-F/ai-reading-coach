...
- Short local Hermes chains inside one `run-daily` may later use a bounded temporary session if Hermes exposes a controllable session/thread API. Cross-day, cross-feedback, and cross-reflection state must be written to Hermes native memory or ARC SQLite.
